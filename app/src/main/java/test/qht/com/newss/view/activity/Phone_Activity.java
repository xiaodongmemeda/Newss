            ...
            case R.id.send_phone:
                if(!TextUtils.isEmpty(phone_edit_n.getText().toString().trim())){
                    if(phone_edit_n.getText().toString().trim().length()==11){
                        ...
                        Toast.makeText(Phone_Activity.this, "请输入完整电话号码", Toast.LENGTH_LONG).show();
                        phone_edit_n.requestFocus();
                    }
                }else{
                    Toast.makeText(Phone_Activity.this, "请输入您的电话号码", Toast.LENGTH_LONG).show();
                    phone_edit_n.requestFocus();
                }
                break;
            case R.id.phone_text_enter:
                if(!TextUtils.isEmpty(phone_edit_y.getText().toString().trim())){
                    if(phone_edit_y.getText().toString().trim().length()==4){
                        ...
                        Toast.makeText(Phone_Activity.this, "请输入完整验证码", Toast.LENGTH_LONG).show();
                        phone_edit_y.requestFocus();
                    }
                }else{
                    Toast.makeText(Phone_Activity.this, "请输入验证码", Toast.LENGTH_LONG).show();
                    phone_edit_y.requestFocus();
                }
                break;
            ...
            if (result == SMSSDK.RESULT_COMPLETE) {
                ...
                if (event == SMSSDK.EVENT_SUBMIT_VERIFICATION_CODE) {//提交验证码成功,验证通过
                   ...
                   /* Message message = Message.obtain();
                    message.obj = phone_edit_y.getText();
                    message.what = 1;
                ...
                if(flag){
                    send_phone.setEnabled(true);
                    Toast.makeText(Phone_Activity.this, "验证码获取失败，请重新获取", Toast.LENGTH_SHORT).show();
                    phone_edit_n.requestFocus();
                }else{
                    ((Throwable) data).printStackTrace();
                    int resId = getStringRes(Phone_Activity.this, "smssdk_network_error");
                    Toast.makeText(Phone_Activity.this, "验证码错误", Toast.LENGTH_SHORT).show();
                    phone_edit_y.selectAll();
                    if (resId > 0) {
                        Toast.makeText(Phone_Activity.this, resId, Toast.LENGTH_SHORT).show();
                    }
                }
